...
- The logo is `public/brand/danvice-logo.svg` — the real Danvice Auto lockup (two swooshes over the DANVICE AUTO wordmark), rendered by `components/layout/Logo.tsx`. One file serves every ground: blue and orange both hold up on white and on `blue-900`, so there is no recoloured dark variant to drift. It carries its own wordmark, so never put HTML text beside it. The wordmark is **Inter Bold, converted to outlines** — no webfont is loaded for it. To regenerate: draw DANVICE / AUTO / OF RSM with fontTools at 1.22× the original cap height, then track each run out so it lands on the same right edge the lockup was drawn around (`(target_width − natural_width) / (glyph_count − 1)`, ≈0.083em for the main run). Inter is much narrower than the Michroma this was set in before, so **track it out, never scale it up** — scaling inflates the cap height and the wordmark starts crowding the orange swoosh.
- `app/icon.svg` is the same mark cropped to the two swooshes; the wordmark is illegible at 16px. `app/opengraph-image.tsx` reads the logo file at build time rather than copying its paths, so the share image cannot drift from the logo. Satori has no network and inherits nothing from the page, so that file also hands it `app/fonts/Inter-{Regular,Bold}.ttf` — Inter subset to Latin-1 plus the punctuation this site sets, 41KB each. `npm run check` fails if anything rendered there falls outside that subset, because a missing glyph is a tofu box in the one image every shared link shows.
...
- **The hero is `min-h-lvh`, never `svh` or `dvh`.** `svh` is the *smallest* viewport, so the moment mobile browser chrome shrinks or floats over the page the hero stops short and the next section's light ground shows as a white band across the bottom of the phone. `dvh` covers but retracks mid-scroll, reflowing an in-flow section (fine for the fixed `MobileNavDrawer`, not for a hero). The copy adds `calc(100lvh - 100svh)` — the chrome height — back as bottom padding so it stays centred in the part you can actually see. All three are asserted.
- **One family, site-wide: Inter via next/font, headings included.** There is no `font-display` class and no `display` family in the Tailwind config; `npm run check` fails if either returns, because the class would name a family that no longer exists and silently render at body weight. Prices and specs use `tabular-nums`.
- **With one family, weight IS the hierarchy.** A serif heading used to announce itself by being a serif; nothing announces it now, so the ladder has to be exact: 700 for `display-*`, 600 for `subhead`/`card-title`/`eyebrow`, 400 for body. All six tokens are asserted. Display type also tracks tighter than it did (−0.035/−0.03/−0.025em) — Fraunces wanted the room, Inter at those sizes reads slack with it.
- Large numerals set outside a heading token — prices, the phone numbers, the financing step numbers — carry an explicit `font-semibold`/`font-bold` and `tracking-tight`. They were leaning on the serif to look like display type; in one family they need saying.
- **Headings use a size token, never a bare `text-lg/xl/2xl`.** Tailwind's own sizes carry no `font-weight`, so a heading set in one silently inherits body 400 while `display-*` sit at 700 — that mismatch flattened the hierarchy on every page. Four tiers: `text-display-*` (page titles), `text-subhead` (a titled block owning a chunk of a page), `text-card-title` (a repeated grid item or in-card widget title), `text-eyebrow`. All carry their own weight. `npm run check` fails on a bare size.
